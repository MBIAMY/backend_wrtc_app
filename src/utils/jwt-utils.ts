...
const envPrivateKeyPath = process.env.JWT_PRIVATE_KEY_PATH;
const defaultPrivateKeyPath = path.join(__dirname, '../', 'environment', 'jwt-private.key');
const privateKey = envPrivateKey
  || (envPrivateKeyPath ? fs.readFileSync(envPrivateKeyPath, { encoding: 'utf-8' }) : fs.readFileSync(defaultPrivateKeyPath, { encoding: 'utf-8' }));
const expiresInSeconds:number = Number(process.env.ACCESS_TOKEN_EXPIRES_IN_SECONDS || (30 * 60));
